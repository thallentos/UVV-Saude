...
const profissionalRepository = {
    ...
    async update(usuario_id: number, data: UpdateProfissionalDTO): Promise<Profissional> {
        const [profissional] = await db<Profissional>('profissionais')
            .where({ usuario_id })
            .update(data)
            .returning('*');
        if (!profissional) throw new Error('Erro ao atualizar profissional.');
        return profissional;
    },
